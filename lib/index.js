...
function plugin(opts) {
  ...
  opts.index.path = opts.index.path || 'tags/index.html';
  opts.index.layout = opts.index.layout || 'partials/tags.hbt';
  opts.index.title = opts.index.title || 'All Tags';
  ...
  return function(files, metalsmith, done) {
    ...
    function subPlaceholders(str, opts) {
      return str
    ...
    for (var tag in tagList) {
      ...
      for (var i = 0; i < numPages; i++) {
        ...
        var textPlaceholders = {
          tag: tag,
          num: page.pagination.num
        };

        // replace placeholder identifiers in any passed-in metadata string values
        for (var key in page) {
          if (typeof page[key] == 'string') {
            // Render the non-first pages differently to the rest, when set.
            if (key == 'path' && i > 0 && opts.pathPage) {
              page[key] = subPlaceholders(opts.pathPage, textPlaceholders);
            }
            page[key] = subPlaceholders(page[key], textPlaceholders);
          }
        }
    ...
    // add index file for all tags
    // set to false to disable
    if (opts.index) {
      files[opts.index.path] = Object.assign(opts.index, { contents: '' });
    }
